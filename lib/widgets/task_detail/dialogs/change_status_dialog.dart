...
class ChangeStatusDialog extends StatefulWidget {
  ...
  final String taskId;
  final task_utils.TaskState currentTaskState;
...
class _ChangeStatusDialogState extends State<ChangeStatusDialog> {
  ...
  void initState() {
    super.initState();
    _selectedStateIndex = task_utils.TaskState.values.indexOf(
  ...
  Widget build(BuildContext context) {
    ...
    final selectedState = task_utils.TaskState.values[_selectedStateIndex];

    String? validationMessage;
    bool canApplyState = true;

    // Lógica de validación
    if (selectedState == task_utils.TaskState.underReview && !widget.hasFiles) {
      canApplyState = false;
      validationMessage = 'Debes subir un archivo para activar "Por Revisar"';
    } else if (selectedState == task_utils.TaskState.done) {
      if (widget.currentTaskState != task_utils.TaskState.underReview) {
        canApplyState = false;
            ...
            SizedBox(
              ...
              child: PageView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  // ...
                  return Center(
                    child: Text(task_utils.TaskState.values[index].name),
                  );
                },
              ),
            ),
